...
- **Reverse proxy** — Caddy (configured in `~/services/Caddyfile`) serves `moodtune.siren.codes` with automatic TLS via Let's Encrypt, proxying to `moodtune-app:3000` on the shared `moodtune` network. The Caddyfile includes security headers (HSTS, X-Content-Type-Options, X-Frame-Options, CSP, Referrer-Policy), exploit path blocking, bot user-agent blocking, JSON access logging to `/var/log/caddy/moodtune-access.log`, and sets `X-Forwarded-For`/`X-Real-IP` headers for accurate client IP logging. A reference copy of the moodtune vhost is kept at `./Caddyfile` (not used at runtime — the canonical version is `~/services/Caddyfile`).
